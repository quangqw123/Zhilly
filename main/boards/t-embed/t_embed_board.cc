...
TEmbedBoard::TEmbedBoard() : boot_button_(BOOT_BUTTON_GPIO), pwr_button_(PWR_BUTTON_GPIO) {
    ...
    gpio_set_direction(LORA_SW0_PIN, GPIO_MODE_OUTPUT);
    gpio_set_level(LORA_SW0_PIN, 1);
    gpio_set_direction(LORA_SW1_PIN, GPIO_MODE_OUTPUT);
...
bool TEmbedBoard::ReadBatteryLevel() {
    ...
    if (err == ESP_OK) {
        ...
        battery_level_ = VoltageToPercentage(vbat);
    }
...
int TEmbedBoard::VoltageToPercentage(float v) {
    struct {
        float volt;
        int pct;
    } table[] = {{4.15f, 100}, {4.05f, 90}, {3.90f, 70}, {3.80f, 50},
                 {3.70f, 30},  {3.60f, 10}, {3.30f, 0}};
    int table_size = sizeof(table) / sizeof(table[0]);

    if (v >= table[0].volt)
        return 100;
    if (v <= table[table_size - 1].volt)
        return 0;
    ...
    for (int i = 0; i < table_size - 1; i++) {
        if (v >= table[i + 1].volt) {
            ...
            int p_high = table[i].pct;
            int p_low = table[i + 1].pct;
            return p_low + (int)((v - v_low) * (p_high - p_low) / (v_high - v_low));
        }
    }
    return 0;
}
