...
void LvglDisplay::UpdateStatusBar(bool update_all) {
    ...
    if (board.GetBatteryLevel(battery_level, charging, discharging)) {
        ...
        if (low_battery_popup_ != nullptr && !update_all) {
            if (discharging && battery_level <= 10) {
                 
                if (battery_level <= last_warned_battery_pct_ - 2 ||
                    last_warned_battery_pct_ > 10) {
                    last_warned_battery_pct_ = battery_level;

                    char buf[64];
                    snprintf(buf, sizeof(buf), "Batarya Çok Düşük! (%%%d)\nLütfen Şarja Takın.",
                             battery_level);
                    lv_label_set_text(low_battery_label_, buf);
